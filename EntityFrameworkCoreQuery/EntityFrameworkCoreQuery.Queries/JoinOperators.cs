...
public class JoinOperators
{
    ...
    /// </summary>
    public static void UsingInclude()
    {
    ...
    public static void UsingIncludeWithFind()
    {
        using var context = new AdventureWorksDbContext();
        ...
        // Can't use query above for FindAsync.
        // FindAsync is not available in IQueryables

        var findme = context.Product.Find(325);

        /*
        info: Microsoft.EntityFrameworkCore.Database.Command[20101]
              Executed DbCommand (43ms) [Parameters=[@__p_0='?' (DbType = Int32)], CommandType='Text', CommandTimeout='30']
              SELECT TOP(1) [p].[ProductID], [p].[Class], [p].[Color], [p].[DaysToManufacture], [p].[DiscontinuedDate], [p].[FinishedGoodsFlag], [p].[ListPrice], [p].[MakeFlag], [p].[ModifiedDate], [p].[Name], [p].[ProductLine], [p].[ProductModelID], [p].[ProductNumber], [p].[ProductSubcategoryID], [p].[ReorderPoint], [p].[rowguid], [p].[SafetyStockLevel], [p].[SellEndDate], [p].[SellStartDate], [p].[Size], [p].[SizeUnitMeasureCode], [p].[StandardCost], [p].[Style], [p].[Weight], [p].[WeightUnitMeasureCode]
              FROM [Production].[Product] AS [p]
              WHERE [p].[ProductID] = @__p_0
        */
    }

    public static void UsingIncludeWithFirstOrDefault()
    {
        using var context = new AdventureWorksDbContext();
        ...
        var findme = query.Where(x => x.ProductId == 325).FirstOrDefault();
        ...
        info: Microsoft.EntityFrameworkCore.Database.Command[20101]
              ...
              LEFT JOIN [Production].[ProductSubcategory] AS [p0] ON [p].[ProductSubcategoryID] = [p0].[ProductSubcategoryID]
              LEFT JOIN [Production].[ProductCategory] AS [p1] ON [p0].[ProductCategoryID] = [p1].[ProductCategoryID]
              WHERE [p].[ProductID] = 325
        */
    }

    public static void UsingQueryMethod()
    {
        using var context = new AdventureWorksDbContext();
        ...
        // FirstOrDefault uses TOP(1)
        var resultFirstOrDefault = query.FirstOrDefault(x => x.product.ProductId == 325);
        ...
        */
        ...
        var resultSingleOrDefault = query.SingleOrDefault(x => x.product.ProductId == 325);

        /*
        info: Microsoft.EntityFrameworkCore.Database.Command[20101]
              Executed DbCommand (12ms) [Parameters=[], CommandType='Text', CommandTimeout='30']
              SELECT TOP(2) [p].[ProductID], [p].[Class], [p].[Color], [p].[DaysToManufacture], [p].[DiscontinuedDate], [p].[FinishedGoodsFlag], [p].[ListPrice], [p].[MakeFlag], [p].[ModifiedDate], [p].[Name], [p].[ProductLine], [p].[ProductModelID], [p].[ProductNumber], [p].[ProductSubcategoryID], [p].[ReorderPoint], [p].[rowguid], [p].[SafetyStockLevel], [p].[SellEndDate], [p].[SellStartDate], [p].[Size], [p].[SizeUnitMeasureCode], [p].[StandardCost], [p].[Style], [p].[Weight], [p].[WeightUnitMeasureCode], [p0].[ProductSubcategoryID], [p0].[ModifiedDate], [p0].[Name], [p0].[ProductCategoryID], [p0].[rowguid], [p1].[ProductCategoryID], [p1].[ModifiedDate], [p1].[Name], [p1].[rowguid]
              FROM [Production].[Product] AS [p]
              LEFT JOIN [Production].[ProductSubcategory] AS [p0] ON [p].[ProductSubcategoryID] = [p0].[ProductSubcategoryID]
              INNER JOIN [Production].[ProductCategory] AS [p1] ON [p0].[ProductCategoryID] = [p1].[ProductCategoryID]
              WHERE [p].[ProductID] = 325
    ...
    public static void UnionDifferentTablesSameTypes()
    {
        using var context = new AdventureWorksDbContext();
    ...
    /// <summary>
    /// This doesn't work; 
    /// It causes an InvalidOperationExceptionError:
    /// Unable to translate set operation when matching columns 
    /// on both sides have different store types.
    /// 
    /// EF cannot translate Description and Name since they have different length
    /// </summary>
    public static void UnionDifferentStoreTypes()
    {
        using var context = new AdventureWorksDbContext();
    ...
    public static void RelatedData()
    {
        //using (var context = new AdventureWorksDbContext())
        //{
